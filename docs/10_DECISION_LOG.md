...
## 2026-07-02: Customer Chat Must Surface Live AI Review Output

Decision: When Live AI review is explicitly enabled, `/customer` must render AI-returned catalog items and AI missing questions in the chat response, while Workflow JSON remains deterministic and validator-backed.
Reason: Owner testing showed the product felt unintelligent because Live AI was not being used from the repo-root server path, and AI catalog/missing-question output was not visible in the customer chat.
Consequences: Studio resolves the monorepo root before reading ignored local OpenAI config. The customer chat now displays `ProductCatalogDraft.items` and BusinessUnderstanding missing questions from Live AI. No provider key is exposed to browser code, no Workflow JSON is generated by AI, and no upload/OCR/RAG/crawler dependency is added.
Revisit trigger: Owner review still finds the conversation too rigid, or real websites require a multi-turn AI builder agent, browser-rendered crawling, or stronger source-backed catalog extraction.

## 2026-07-02: Customer Text Send Must Be Message-First

Decision: In `/customer`, pressing Send uses only the message text or a URL typed inside that message. A URL left in the separate website field is crawled only when the owner clicks `Use link`.
